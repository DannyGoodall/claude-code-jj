...
| Skill: `jj-absorb` | `plugins/jj-concurrent/skills/jj-absorb/SKILL.md` | Amend-after-review — preview with `jj absorb --dry-run`, distribute scattered working-copy hunks into the downstack commit that last touched those lines, report where each landed, leave ambiguous hunks for manual placement. Orchestrator-only. |
| Skills: `jj-checkpoint` / `jj-rewind` | `plugins/jj-concurrent/skills/jj-checkpoint/`, `…/jj-rewind/` | Op-log safety net — `/jj-checkpoint <label>` records the current `jj op` id in the manifest before a risky step; `/jj-rewind [label]` rolls the whole repo back to it via `jj op restore` after a confirmation summary. Orchestrator-only. |

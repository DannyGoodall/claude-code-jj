# jj-op-checkpoint Specification

## Purpose
TBD - created by archiving change jj-op-checkpoint. Update Purpose after archive.
## Requirements
### Requirement: Record a labelled checkpoint before a risky step

The plugin SHALL provide a single orchestrator skill exposing two verbs — record and rewind. `/jj-checkpoint <label>` (the record verb, the default) captures the repository's current operation id (resolved from `jj op log`) under the human-supplied `<label>` and persists it as a checkpoint record. A checkpoint record SHALL contain at minimum the label, the captured op id, a timestamp, and an optional note. Recording a checkpoint SHALL be read-only with respect to repository history — it SHALL NOT itself create, abandon, or restore any operation; it only remembers the current op id so a later rewind can return to it. The skill SHALL run only in the orchestrator role (which owns the agent-plan manifest) and SHALL NOT be invoked inside a worker.
...
The same skill SHALL provide a rewind verb — `/jj-checkpoint rewind [label]` — that restores the repository to a checkpoint's captured operation id via `jj op restore <op-id>`. When a `label` is given, the rewind SHALL target that named checkpoint; when no label is given, it SHALL target the most recently recorded checkpoint. The rewind SHALL resolve the label to its stored op id and SHALL restore to exactly that op. The skill's description SHALL retain the rewind trigger phrases (e.g. "undo back to the checkpoint", "roll back that rebase", "restore to <label>") so rewind intent still routes to the skill now that `/jj-rewind` is no longer a standalone skill. The rewind verb SHALL run only in the orchestrator role and SHALL NOT be invoked inside a worker.

#### Scenario: Rewind to a named checkpoint

- **WHEN** `/jj-checkpoint rewind pre-fanout-integration` is invoked and that checkpoint exists
...
- **WHEN** `/jj-checkpoint rewind` is invoked with no label
...
- **WHEN** the rewind verb is invoked for a label with no checkpoint record
...
Both verbs of the skill SHALL operate exclusively through jj's operation log — `jj op log` to read and resolve op ids and `jj op restore` to roll back — and SHALL run non-interactive jj only (`--no-pager`, no editor spawn, no `-i`/`--interactive`). Neither verb SHALL delete or remove `.jj` (or any repository metadata), SHALL run raw mutating git, or SHALL touch bookmarks or push directly; rollback is achieved by `jj op restore` alone, which is itself recorded as a new operation and is therefore reversible.
...
- **WHEN** the rewind verb restores to an earlier op
...
The skill SHALL be usable as the before/after-a-risky-step beats of the `jj-delegate` reconcile/integration lifecycle: `/jj-checkpoint <label>` invoked immediately before a fan-out integration or a large rebase, and `/jj-checkpoint rewind [label]` invoked to undo that step if it goes wrong. Checkpoint records SHALL be persisted alongside the orchestrator's existing agent-plan manifest so they survive across orchestration steps within a session.
...
- **AND** if the integration goes wrong it invokes `/jj-checkpoint rewind <label>` to return to that point
...
- **THEN** the checkpoint remains resolvable by its label for a later rewind

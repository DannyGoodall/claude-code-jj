...
The skill SHALL run `jj absorb --dry-run` and surface the planned placement (which hunk would land in which commit) before performing any mutating `jj absorb`. The preview SHALL be produced first so the orchestrator can confirm the plan; when the dry-run shows that nothing can be absorbed, the skill SHALL report that and SHALL NOT run a mutating absorb.

#### Scenario: Preview is produced before commits move

- **WHEN** `/jj-absorb` is invoked
...
#### Scenario: Nothing to absorb is reported without mutating

- **WHEN** the dry-run shows no hunk has a downstack home
- **THEN** the skill reports "nothing to absorb"
- **AND** it does not run a mutating `jj absorb`
...
The skill SHALL run only in the orchestrator role, operating on the orchestrator's own change stack. It SHALL NOT move bookmarks, perform `jj git push`, or run raw mutating git, and SHALL run every command non-interactively (`--no-pager`; no `-i`/`--interactive`; no editor spawn). The skill SHALL preflight-check that the installed `jj` supports `jj absorb` (and `jj absorb --dry-run`) and SHALL report a clear blocker when it does not, rather than hanging or improvising.
...
- **WHEN** the installed `jj` does not support `jj absorb`
